...
```bash
script_optimizer <opções_do_otimizador> -- <executável_aplicação> <parâmetros_da_aplicação>
````
...
```bash
user$ script_optimizer -r -s script.sh -j teste-customizado -n 1:10 -p 1 2 4 -t 2 4 8 16 3
...
```bash
user$ script_optimizer -r -s script.sh -j teste -- raxml -N 100 -s DENV_3-colombia-BVBRC_genome_sequence.mafft -m GTRGAMMA -p 112233 -b 223344 -n teste_raxml.phylip_tree2.raxml -c 4 -f d
➡️  Script de submissão script.sh criado com sucesso!
➡️  Script de submissão submetido com sucesso!
