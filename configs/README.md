...
- $\color{blue}\text{\textbf{slurm}}$: Objeto com as informações sobre como o script deve ser submetido. Tem os seguintes campos:
  ...
  - $\color{blue}\text{\textbf{submission\\\_message}}$: Expressão regular que define como o ID do job deve ser extraído da mensagem gerada pelo programa de submissão no stdout. No exemplo do arquivo de configuração, a espressão regular, para capturar o ID do job na saída do sbatch é $\color{gray}\text{\textbf{.*Submitted batch job (\\\textbackslash\\\textbackslash+).*}}$.

## Exemplo do arquivo de configuração $\color{red}\text{\textbf{predictors\\\_info.json}}$

```json
{
  "raxml-sscad": "raxml-sscad_ETR_EDP.pickle",
  "raxml-sscad-time": "raxml-sscad-time_ETR_ElapsedRaw.pickle",
  "raxml": "raxml_ETR_EDP.pickle",
  "ntb": "ntb_RFR_EDP.pickle"
}
```

O arquivo de configuração do preditor é gerado automaticamente peloscipt de treinamento. É armazenado no diretório $\color{blue}\text{\textbf{predictors\\\_path}}$ definido no arquivo $\color{red}\text{\textbf{system\\\_config.json}}$ de configuração do sistema, e o seu nome é definido em $\color{blue}\text{\textbf{predictors\\\_info\\\_config\\\_filename}}$ também neste arquivo de configuração. Cada campo do objeto é no formato $\color{gray}\text{\textbf{app: predictor\\\_name}}$, onde $\color{gray}\text{\textbf{app}}$ é o nome da aplicação, como identificado quando o treinamento é feito para a aplicação e $\color{gray}\text{\textbf{predictor\\\_name}}$ é o nome do preditor gerado para a aplicação após o treinamento. Todos os preditores também são armazenados no diretório $\color{blue}\text{\textbf{predictors\\\_path}}$. Por exemplo, a entrada $\color{gray}\text{\textbf{``raxml'':}}$$\color{gray}\text{\textbf{``raxml\\\_ETR\\\_EDP.pickle''}}$ indica que, para a aplicação identificada por $\color{gray}\text{\textbf{raxml}}$ (a do arquivo de configuração dado e descrito anteriormente), o preditor gerado após os treinamentos, que será usado para fazer as sugestões e predizer o tempo de execução da aplicação quando os usuários usarem o script de otimização escolhendo esta aplicação, está armazenado no arquivo $\color{gray}\text{\textbf{raxml\\\_ETR\\\_EDP.pickle}}$.
