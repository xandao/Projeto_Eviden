...
class ReadSystemConfig:
  ...
  def read_system_config(self, system_config_path):
    """
    Função para ler o arquivo de configuração do sistema a partir do 
    ...
    try:
      ...
      # salva o JSON convertido para dicionário em self.system_config.
...
class ReadTrainingConfig:
  ...
  hiperparâmetros que otimizaremos, parâmetros fixos (por exemplo, 
  randomstate=42) e outras informações como o nome do modelo e como ele 
  é acessado no python (para o import dinâmico).

  Atributos:
    training_config_path (Path | None): Caminho completo para o arquivo
                                        de configuração dos modelos a 
                                        serem treinados.
  ...
  def read_training_config(self, training_config_path):
    ...
    try:
      # Tenta converter o JSON, verificando a sintaxe do formato, e 
      # salva o JSON convertido para dicionário em self.training_config.
...
class ReadApplicationsConfigs:
  ...
  def read_applications_config(self, applications_config_dir):
    ...
    if applications_files:
      ...
      try:
        # Para cada possível arquivo no diretório 
        # applications_config_dir.
        for file in applications_files:
            # Se file realmente for um arquivo, lemos o seu conteúdo, 
            # verificando se é um JSON válido e depois verificamos a 
            # validade do arquido de acordo com o esquema.
            if file.is_file():
              ...
              # Salva o arquivo JSON da aplicação convertido par um 
              ...
              # informando sobre este erro (file sempre deveria ser um 
              # arquivo).
...
class ReadUserConfig:
  ...
  def read_user_config(self, user_config_path):
    ...
    Parâmetros:
      user_config_path (Path): Caminho completo do arquivo de 
    ...
    try:
      # Tenta converter o JSON, verificando a sintaxe do formato, e
      # salva o JSON convertido para dicionário em self.user_config.
...
class PredictorsInfoConfig:
  ...
  def read_predictors_info_config(self, predictors_info_config_path):
    ...
    try:
      # Tenta converter o JSON, verificando a sintaxe do formato, e
      # salva o JSON convertido para dicionário em 
      ...
      if predictors_info_config_path.is_file():
        ...
        # O arquivo somente pode não existir se ainda não treinamos 
        # nenhum modelo para nenhuma aplicação, ou seja, somente pode 
        # ocorrer no script de treinamento. No script de otimização, o
      ...
      # Retorna a configuração dos preditores lida e e convertida para 
      # um dicionário.  
